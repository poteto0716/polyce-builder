...
4. Optionally verify the bundled data with `python3 scripts/external_data.py verify pcff`
   and `python3 scripts/external_data.py verify iff`.
...
For the silica interface, install LAMMPS with the additional KSPACE and EXTRA-FIX
features. The required IFF data are already bundled. `./run.sh` creates independent components;
...
If an example reports missing or modified external files, restore them using the
[external-data document](external_data.md). If the executable reports `GLIBC` or `GLIBCXX` version errors,
